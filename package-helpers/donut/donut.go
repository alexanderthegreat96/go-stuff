...
func get_win_size() (*winsize, error) {
	fd := int(os.Stdout.Fd())
	height, width, err := term.GetSize(fd)
	if err != nil {
		return nil, err
	}
	return &winsize{
		Row:    uint16(height),
		Col:    uint16(width),
		Xpixel: 0, // You can set this to 0 or a suitable value.
		Ypixel: 0, // You can set this to 0 or a suitable value.
	}, nil
}
