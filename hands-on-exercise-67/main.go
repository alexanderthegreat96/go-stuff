...
// Testing
// Using interfaces and structs to mock databases
...
// This type contains a type of Users that is a map id -> name
type MockDatastore struct {
	Users map[int]User // bind to the User struct via their ID
}

// we attach the data store to the type MockDataStore
func (md MockDatastore) GetUser(id int) (User, error) {
	user, ok := md.Users[id] // if user is found
	if !ok {                 // hence why we check if not ok
		return User{}, fmt.Errorf("User %v not found", id) // return an empty User type
	}
	return user, nil // error will be nil and we have the user
}

// same as above, the difference is, we want the user to not exist
// in order to save
